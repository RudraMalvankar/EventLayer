...
async function fetchJson(url) {
  const response = await fetch(url, { headers: { 'User-Agent': 'TechPulse/1.0' } })
...
async function scrapeFromApi() {
  const [listRes, searchRes] = await Promise.all([
    fetchJson('https://api.lu.ma/public/v1/calendar/list-events?pagination_limit=50'),
    fetchJson('https://api.lu.ma/public/v1/event/search?query=hackathon+mumbai')
  ])
  const rawEvents = [...(listRes?.entries || []), ...(searchRes?.entries || [])]
  return rawEvents
    .map((event) =>
      normalizeEvent(
        {
          ...
          url: event.url,
          start_at: event.start_at || event.starts_at || event.startDate || event.start_date,
          end_at: event.end_at || event.ends_at || event.endDate || event.end_date,
...
async function scrapeFromHtml(url = 'https://luma.com/mumbai') {
  ...
  $('a[href]').each((_, element) => {
    ...
    const title = normalizeText($(element).text()) || normalizeText(card.find('h1,h2,h3').first().text())
    const rawText = normalizeText(card.text())
    const organizer = normalizeText((rawText.split('By')[1] || '').split(/Sold Out|\+\d+/)[0])
