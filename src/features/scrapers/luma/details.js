...
function collectSectionText($, labelRegex) {
  const heading = $("h1,h2,h3,h4,div,p,strong,span")
    .filter((_, el) => labelRegex.test(normalizeText($(el).text())))
    .first();
...
function findTicketLink($) {
  const link = $("a[href]")
    .filter((_, el) => {
      ...
      return (
        /ticket|register|rsvp/i.test(text) || /ticket|register|rsvp/i.test(href)
      );
    })
    .first();
