...
export async function scrapeUnstop() {
  try {
    ...
    $(".challenge-card, .double-card, .event-card, a[href]").each((_, el) => {
      ...
      const title = $el.find(".challenge-name, .event-title, h2, h3").text().trim() || $el.text().trim();
      if (!title || title.length < 5) return;
      
      const url = href.startsWith("http") ? href : `https://unstop.com${href}`;
      const img = $el.find("img").attr("src") || $el.find(".event-banner img").attr("src");
      const dateStr = $el.find(".date, .event-date, .registration-status").text().trim();
      
      found.push({ 
        title, 
        url,
        image: img ? (img.startsWith("http") ? img : `https://unstop.com${img}`) : null,
        dateStr
      });
    });
